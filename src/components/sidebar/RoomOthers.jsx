    ...
    useEffect(() => {
        const fetchRooms = async () => {
            try {
                ...
                const filteredRooms = response.data.filter(room => room.ownerName !== user.username);
        ...
        fetchRooms();
    }, [user]); // Add user to dependency array to refetch rooms when user changes
                    ...
                    <div id="memberContainer" className="flex space-x-6 py-2 px-3 cursor-pointer" style={{ scrollSnapType: 'x mandatory', overflowX: 'scroll', overflowY: 'hidden', whiteSpace: 'nowrap' }}>
                        {room.members.map((member, index) => (
                            <div key={index} className="flex-shrink-0 flex flex-col items-center">
                                <img src="https://img.freepik.com/free-psd/3d-illustration-human-avatar-profile_23-2150671142.jpg" alt="Avatar" className="w-12 h-12 rounded-full" />
