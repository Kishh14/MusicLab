...
/**
 * @type {React.FC<import("../../types/user").RoomType>}
 */

const RoomOwner = ({ name, owner, members, isLocked }) => {
  const { user } = useAuth();
  ...
  const handleToggleRoomLock = () => {
    setRoom((prev) => ({ ...prev, isLocked: !prev.isLocked }));
  };

  return (
    <div
      className={`mt-4 -mx-3 space-y-6 cursor cursor-pointer rounded-lg h-[172px] border-2 ${
