...
const Chat = ({ currentRoom }) => {
  ...
  return (
    <div className="mt-4 -mx-3 space-y-1 cursor-pointer h-[450px] rounded-lg border-gray-500 border flex flex-col">
      <div className="overflow-hidden flex-grow">
        <div className="overflow-y-auto h-full py-4" ref={chatContainerRef}>
          {messages.map(message => (
            <div
              key={message._id}
              className={`flex items-center flex-row ${message.sender.username === user.username ? 'justify-end' : 'justify-start'}`}
            >
              {message.sender.username === user.username ?
                <>
                  <div className={`bg-gray-700 rounded-lg border p-2 mb-2  overflow-hidden text-left`}>
                    {message.text}
                  </div>
                ...
                <>
                  <div className="flex flex-col">
                    <div>
                      ...
                      <div
                        className="bg-gray-700 rounded-lg border p-2 mb-2  ml-2  text-left"
                      >
                        {message.text}
                      </div>
                    </div>
